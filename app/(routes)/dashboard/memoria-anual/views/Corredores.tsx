...
// Interfaz para los datos de movimientos (probablemente ya la tienes)
...
// Nuevas interfaces para los datos de evolución y concentración de corredores
interface CorredorEvolutionItem {
  periodo: string;
  total_clp: number;
  total_uf: number;
  num_corredores: number;
...
interface CorredorConcentracionItem {
  periodo: string;
  hhi_general: number;
  grupo: string; // o number, verifica el tipo en tu base de datos/API
  hhi_grupo: number;
  // Añade otros campos si existen
}

// Interfaz combinada para todos los datos históricos de corredores
interface HistoricalCorredoresAllData {
  evolution: CorredorEvolutionItem[];
  concentracion: CorredorConcentracionItem[];
...
export default function CorredoresView() {
  ...
  const [historicalData, setHistoricalData] = useState<HistoricalCorredoresAllData | null>(null);
    ...
    <div className={`${inter.variable} ${spaceGrotesk.variable} font-sans`}>
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          ...
          {historicalData && historicalData.evolution && historicalData.evolution.length > 0 ? (
            <ChartCountEvolution 
