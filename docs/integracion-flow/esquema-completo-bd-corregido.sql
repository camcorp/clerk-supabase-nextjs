-- =====================================================
-- ESQUEMA COMPLETO DE BASE DE DATOS - CORREGIDO
...
RETURNS TRIGGER AS $$
BEGIN
    -- Verificar dinámicamente si la columna fecha_actualizacion existe
    IF EXISTS (
...
-- 11. CREAR TRIGGERS PARA AUDITORÍA (solo para tablas que tienen fecha_actualizacion)
-- Trigger para corredores
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'corredores' AND column_name = 'fecha_actualizacion') THEN
        DROP TRIGGER IF EXISTS trigger_actualizar_fecha_corredores ON corredores;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'productos' AND column_name = 'fecha_actualizacion') THEN
...
-- Trigger para accesos_usuarios
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'accesos_usuarios' AND column_name = 'fecha_actualizacion') THEN
        DROP TRIGGER IF EXISTS trigger_actualizar_fecha_accesos ON accesos_usuarios;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pagos' AND column_name = 'fecha_actualizacion') THEN
        DROP TRIGGER IF EXISTS trigger_actualizar_fecha_pagos ON pagos;
...
-- Trigger para configuracion_flow
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'configuracion_flow' AND column_name = 'fecha_actualizacion') THEN
        DROP TRIGGER IF EXISTS trigger_actualizar_fecha_configuracion_flow ON configuracion_flow;
        CREATE TRIGGER trigger_actualizar_fecha_configuracion_flow
            BEFORE UPDATE ON configuracion_flow
            FOR EACH ROW
            EXECUTE FUNCTION actualizar_fecha_modificacion();
    END IF;
END $$;
...
-- NOTA: precio_bruto se omite porque es una columna generada
INSERT INTO productos (codigo, nombre, descripcion, precio_neto, iva, tipo_producto, duracion_dias, activo)
VALUES (
    'rp_001',
    'Reporte Individual',
...
-- NOTA: Esta función se crea al final para asegurar que todas las tablas existan
CREATE OR REPLACE FUNCTION limpiar_datos_expirados()
RETURNS INTEGER AS $$
DECLARE
    registros_eliminados INTEGER := 0;
    reportes_actualizados INTEGER := 0;
    accesos_actualizados INTEGER := 0;
BEGIN
    -- Verificar si las tablas existen antes de intentar actualizarlas
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reportes_individuales') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'accesos_usuarios') THEN
        ...
        GET DIAGNOSTICS accesos_actualizados = ROW_COUNT;
    END IF;
    
    -- Calcular total de registros procesados
    registros_eliminados := reportes_actualizados + accesos_actualizados;
...
-- =====================================================
-- SCRIPT COMPLETADO - VERSIÓN CORREGIDA
-- =====================================================

-- CAMBIOS EN ESTA VERSIÓN:
-- 1. Removido precio_bruto del INSERT porque es columna generada
-- 2. Removido precio_bruto del UPDATE en ON CONFLICT
-- 3. Mantenida la funcionalidad completa del sistema
